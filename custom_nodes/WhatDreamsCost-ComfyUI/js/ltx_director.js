...
const CANVAS_HEIGHT = RULER_HEIGHT + BLOCK_HEIGHT + CAMERA_TRACK_HEIGHT + CONTROL_TRACK_HEIGHT + AUDIO_TRACK_HEIGHT;
...
function parseInitial(jsonStr) {
  let parsed = { segments: [], referenceImages: [], cameraSegments: [], controlSegments: [], audioSegments: [] };
  try {
    if (jsonStr) {
...
class TimelineEditor {
  constructor(node, container, domWidget) {
    ...
    // Core data
    this.timeline = { segments: [], referenceImages: [], cameraSegments: [], controlSegments: [], audioSegments: [] };
    this.selectionType = "image"; // "image", "reference", "camera", "control", or "audio"
  ...
  getTrackY(track) {
    if (track === "camera") return RULER_HEIGHT + this.blockHeight;
    if (track === "control") return RULER_HEIGHT + this.blockHeight + this.cameraTrackHeight;
    if (track === "audio") return RULER_HEIGHT + this.blockHeight + this.cameraTrackHeight + this.controlTrackHeight;
    return RULER_HEIGHT;
  }
  ...
  getTrackTypeAtY(y) {
    ...
    if (y <= RULER_HEIGHT + this.blockHeight + this.cameraTrackHeight) return "camera";
    if (y <= RULER_HEIGHT + this.blockHeight + this.cameraTrackHeight + this.controlTrackHeight) return "control";
    return "audio";
  }
  ...
  setTrackArray(track, arr) {
    if (track === "audio") this.timeline.audioSegments = arr;
  ...
  createDOM() {
    ...
    // --- Text Area (Keyframe/Text/Camera/Reference) ---
    ...
    this.promptInput.addEventListener("input", () => {
      if (this.selectionType === "image" && this.timeline.segments[this.selectedIndex]) {
        ...
        this.commitChanges();
      } else if (this.selectionType === "camera" && this.timeline.cameraSegments[this.selectedIndex]) {
        this.timeline.cameraSegments[this.selectedIndex].prompt = this.promptInput.value;
        this.commitChanges();
    ...
    this.wrapper.addEventListener("dragover", (e) => {
      ...
      const isAudioTrack = this.getTrackTypeAtY(y) === "audio";
  ...
  render() {
    ...
    this.ctx.fillRect(0, RULER_HEIGHT + this.blockHeight, width, this.cameraTrackHeight);
    this.ctx.fillStyle = "#181122"; // IC-LoRA control track bg
    this.ctx.fillRect(0, RULER_HEIGHT + this.blockHeight + this.cameraTrackHeight, width, this.controlTrackHeight);
    this.ctx.fillStyle = "#111"; // Audio track bg
    this.ctx.fillRect(0, RULER_HEIGHT + this.blockHeight + this.cameraTrackHeight + this.controlTrackHeight, width, this.audioTrackHeight);
    ...
    let renderSegments = (this._previewSegments && !previewIsAudio && !previewIsCamera && !previewIsControl)
    ...
    for (let i = 0; i < sortedControlSegments.length; i++) {
      ...
      const trackY = RULER_HEIGHT + this.blockHeight + this.cameraTrackHeight;
    ...
    for (let i = 0; i < sortedAudioSegments.length; i++) {
      ...
      const trackY = RULER_HEIGHT + this.blockHeight + this.cameraTrackHeight + this.controlTrackHeight;
    ...
    this.ctx.fillRect(0, RULER_HEIGHT + this.blockHeight + this.cameraTrackHeight, width, 1);
    this.ctx.fillRect(0, RULER_HEIGHT + this.blockHeight + this.cameraTrackHeight + this.controlTrackHeight, width, 1);
    ...
    const grabBarY = RULER_HEIGHT + (this.blockHeight + this.cameraTrackHeight + this.controlTrackHeight + this.audioTrackHeight - grabBarH) / 2;
  ...
  drawCameraSegmentVisuals(ctx, seg, isSelected, yOffset, trackHeight, startX, pxWidth) {
    ...
    let text = seg.prompt || "Camera Control";
  ...
  onMouseMove(e) {
    ...
    if (this._dragType === "divider") {
      ...
      this.canvasHeight = this.rulerHeight + this.blockHeight + this.cameraTrackHeight + this.controlTrackHeight + this.audioTrackHeight;
  ...
  commitChanges(skipRender = false) {
    ...
    for (let seg of sortedSegments) {
      ...
      if (seg.start > currentCursor) {
        if (baseRanges.length > 0) baseRanges[baseRanges.length - 1].end = Math.min(seg.start, durationFrames);
      }

      const rangeStart = baseRanges.length === 0 ? 0 : seg.start;
      const clippedEnd = Math.min(seg.start + seg.length, durationFrames);
      if (clippedEnd > rangeStart) {
        baseRanges.push({ start: rangeStart, end: clippedEnd, prompt: seg.prompt || "" });
      }
      currentCursor = seg.start + seg.length;
    }

    const clampedCursor = Math.min(currentCursor, durationFrames);
    if (baseRanges.length > 0 && clampedCursor < durationFrames) {
      baseRanges[baseRanges.length - 1].end = durationFrames;
    }

    // Allow camera-only prompting for T2V / prompt-only director work.
    if (baseRanges.length === 0 && (
      sortedCameraSegments.some(s => s.start < durationFrames && (s.prompt || "").trim()) ||
      sortedControlSegments.some(s => s.start < durationFrames && ((s.prompt || "").trim() || s.controlType))
    )) {
      baseRanges.push({ start: 0, end: durationFrames, prompt: "" });
    }

    const contiguousLengths = [];
    const contiguousPrompts = [];
    const cameraLabel = (prompt) => prompt ? `Camera: ${prompt}` : "";
    ...
    for (const base of baseRanges) {
      const cuts = new Set([base.start, base.end]);
      for (const cam of sortedCameraSegments) {
        const camStart = Math.max(0, cam.start);
        const camEnd = Math.min(durationFrames, cam.start + cam.length);
        if (camEnd <= base.start || camStart >= base.end) continue;
        cuts.add(Math.max(base.start, camStart));
        cuts.add(Math.min(base.end, camEnd));
      }
      for (const ctrl of sortedControlSegments) {
        const ctrlStart = Math.max(0, ctrl.start);
        const ctrlEnd = Math.min(durationFrames, ctrl.start + ctrl.length);
        if (ctrlEnd <= base.start || ctrlStart >= base.end) continue;
        cuts.add(Math.max(base.start, ctrlStart));
        cuts.add(Math.min(base.end, ctrlEnd));
      }
      ...
      for (let i = 0; i < orderedCuts.length - 1; i++) {
        ...
        const parts = [(base.prompt || "").trim(), ...activeCameraPrompts, ...activeControlPrompts, ...referenceHints].filter(Boolean);
        contiguousLengths.push(end - start);
        contiguousPrompts.push(parts.join(". "));
      }
    }
    ...
    const toSave = {
      ...
      cameraSegments: (this.timeline.cameraSegments || []).map(s => ({ ...s })),
  ...
  onContextMenu(e) {
    ...
    } else if (isAudioTrack || isImageTrack || isCameraTrack || isControlTrack) {
      const gapRegions = this.getGapRegions();
  ...
  showContextMenu(clientX, clientY, seg, trackType) {
    ...
    const isImage = trackType !== "audio" && trackType !== "camera" && trackType !== "text" && seg.imageB64;
    ...
    copySegBtn.onclick = () => {
      ...
      this._copiedSegmentTrack = trackType === "audio" ? "audio" : (trackType === "control" ? "control" : (trackType === "camera" ? "camera" : "image"));
      this.dismissContextMenu();
    };
    menu.appendChild(copySegBtn);

    const currentTrack = trackType === "audio" ? "audio" : (trackType === "control" ? "control" : (trackType === "camera" ? "camera" : "image"));
  ...
  showGapContextMenu(clientX, clientY, gap) {
    ...
    const currentTrack = gap.track === "audio" ? "audio" : (gap.track === "control" ? "control" : (gap.track === "camera" ? "camera" : "image"));
  ...
  addSegmentInGap(frameStart, frameEnd, type = "text") {
    ...
    const track = type === "control" ? "control" : (type === "camera" ? "camera" : "image");
    if (type === "control") {
  ...
  addCameraSegmentFreeSpace() {
    ...
    const seg = {
      ...
      prompt: "locked stable camera, no shake, subject centered",
      type: "camera",
    };
